...
//
// Gen2 (gen2/rate_update_engine_test.cc).

// This defines all the objects needed for setup and testing
class Gen3RateUpdateEngineTest : public FalconTestingHelpers::FalconTestSetup,
                                 public testing::Test {
 protected:
  void SetUp() override {
    FalconConfig config =
  ...
  ProtocolRateUpdateEngineTestPeer<falcon_rue::EVENT_Gen3,
                                   falcon_rue::Response_Gen3>
      peer_;
};

// Tests the initialization of the Gen3 fields in CongestionControlMetadata.
TEST_F(Gen3RateUpdateEngineTest, CongestionControlMetadataInitialization) {
  ...
  uint32_t num_flows = flow_labels.size();
  uint32_t expected_initial_flow_fcwnd =
      falcon_rue::FloatToFixed<double, uint32_t>(
          falcon_->get_config()->rue().initial_fcwnd() / num_flows,
          falcon_rue::kFractionalBits);

  // Check ConnectionState::CongestionControlMetadata initialization is as
  // expected.
  EXPECT_EQ(ccmeta.gen2_plb_state, 0);
  EXPECT_EQ(flow_fcwnds.size(), flow_labels.size());
  EXPECT_EQ(num_flows,
  ...
  for (int idx = 0; idx < num_flows; idx++) {
    ...
    EXPECT_EQ(flow_fcwnds[idx], expected_initial_flow_fcwnd);
...
// Tests that rate control in Gen3 RUE is at the <connection, flow> control
// level.
TEST_F(Gen3RateUpdateEngineTest, RateControlIsAtConnectionFlowLevel) {
