...
#ifndef ISEKAI_HOST_FALCON_RUE_FORMAT_GEN3_H_
#define ISEKAI_HOST_FALCON_RUE_FORMAT_GEN3_H_
...
// The comment above a variable refers to the name for the field
// Changes from Gen_2:
// - Increased retx_reason width from 1 to 2 bits.
// - Move the position of is_ipv4.
struct __attribute__((packed)) EVENT_Gen3 {
  // cid
  ...
  // retx_reason
  falcon::RetransmitReason retransmit_reason : kGen3RetransmitReasonBits;
...
// - Added pto.
// - Added rack_rto.
// - Added cwnd_carryover.
struct __attribute__((packed)) Response_Gen3 {
  // cid
  uint32_t connection_id : kConnectionIdBits;
  ...
  // flow_weight[3:0]
  uint8_t flow_weight_1 : kFlowLabelWeightBits;
  uint8_t flow_weight_2 : kFlowLabelWeightBits;
  uint8_t flow_weight_3 : kFlowLabelWeightBits;
  uint8_t flow_weight_4 : kFlowLabelWeightBits;
  ...
  uint8_t event_queue_select : kEventQueueSelectBits;
  // pto
  uint32_t tlp_probe_timeout : kTimeBits;
  // rack_rto
  uint32_t rack_retransmit_timeout : kTimeBits;
  uint8_t cwnd_carryover : kCwndCarryoverBits;
  uint64_t padding_0 : 25;

  std::string ToString() const {
    std::ostringstream stream;
    stream << "Gen_3 Response: " << "\n---begin---"
           << "\n  connection_id   : " << connection_id
           << "\n  fabric_cwnd     : " << fabric_congestion_window
           ...
           << "\n  flow_weight_1   : " << static_cast<int>(flow_weight_1)
           << "\n  flow_weight_2   : " << static_cast<int>(flow_weight_2)
           << "\n  flow_weight_3   : " << static_cast<int>(flow_weight_3)
           << "\n  flow_weight_4   : " << static_cast<int>(flow_weight_4)
           ...
           << "\n  event_q_select  : " << static_cast<int>(event_queue_select)
           << "\n  tlp_pto         : " << tlp_probe_timeout
           << "\n  rack_rto        : " << rack_retransmit_timeout
           << "\n  cwnd_carryover  : " << static_cast<int>(cwnd_carryover)
...
static_assert(sizeof(EVENT_Gen3) == ABSL_CACHELINE_SIZE,
              "Event is not one cache line :(");
static_assert(sizeof(Response_Gen3) == ABSL_CACHELINE_SIZE,
              "Response is not one cache line :(");

}  // namespace falcon_rue

#endif  // ISEKAI_HOST_FALCON_RUE_FORMAT_GEN3_H_
